...
static inline bool inc_ptr(editbuf_t *eb, uint8_t **p) {
    uint8_t *p_old = *p;
...
static inline bool dec_ptr(editbuf_t *eb, uint8_t **p) {
    uint8_t *p_old = *p;
...
static uint8_t *get_line_addr(editbuf_t *eb, int line) {
...
static void invalidate_cached(editbuf_t *eb) {
    eb->cached_p      = eb->p_buf;
    eb->cached_p_line = 0;
}
...
static int get_line_len(editbuf_t *eb, const uint8_t *p_line) {
...
static uint8_t *get_line_end(editbuf_t *eb, uint8_t *p_line) {
    uint8_t *p = p_line;
...
static uint8_t *move_split_after_line(editbuf_t *eb, int line) {
...
bool editbuf_delete_range(editbuf_t *eb, location_t loc_from, location_t loc_to) {
