...
static inline uint32_t rc_bittree(struct rc_dec *rc, uint16_t *probs, uint32_t limit) {
    uint32_t symbol = 1;
...
static inline void rc_bittree_reverse(struct rc_dec *rc, uint16_t *probs, uint32_t *dest, uint32_t limit) {
    uint32_t symbol = 1;
    uint32_t i      = 0;
...
static struct xz_dec_lzma2 *xz_dec_lzma2_create(uint32_t dict_max) {
 ...
 *
 * This can return XZ_INTERNAL_OK (more input needed), XZ_SUCCESS (everything
...
static enum xz_ret dec_stream_header(struct xz_dec *s) {
    if (memcmp(s->temp.buf, HEADER_MAGIC, HEADER_MAGIC_SIZE) != 0)
...
static enum xz_ret dec_stream_footer(struct xz_dec *s) {
    if (memcmp(s->temp.buf + 10, FOOTER_MAGIC, FOOTER_MAGIC_SIZE) != 0)
        return XZ_DATA_ERROR;
    ...
    /*
     * Use XZ_SUCCESS instead of XZ_INTERNAL_OK to be more convenient
     * for the caller.
     */
    return XZ_SUCCESS;
}
...
static enum xz_ret dec_block_header(struct xz_dec *s) {
    ...
    if (s->temp.buf[1] & 0x40) {
        if (dec_vli(s, s->temp.buf, &s->temp.pos, s->temp.size) != XZ_SUCCESS)
            return XZ_DATA_ERROR;
...
static struct xz_dec *xz_dec_init(uint32_t dict_max) {
