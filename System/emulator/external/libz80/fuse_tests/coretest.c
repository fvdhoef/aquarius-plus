...
static uint8_t context_mem_read_callback(size_t param, uint16_t address) {
    return memory[address];
}

static void context_mem_write_callback(size_t param, uint16_t address, uint8_t data) {
    memory[address] = data;
}

static uint8_t context_io_read_callback(size_t param, uint16_t address) {
    uint8_t data = address >> 8;
    printf("PR %04x %02x\n", address, data);
    return data;
}

static void context_io_write_callback(size_t param, uint16_t address, uint8_t data) {
    printf("PW %04x %02x\n", address, data);
}
...
read_test(FILE *f, unsigned *end_tstates) {
    ...
    if (fscanf(f, "%x %x %x %x %x %x %x %x %x %x %x %x", &af, &bc, &de, &hl, &af_, &bc_, &de_, &hl_, &ix, &iy, &sp, &pc) != 12) {
    ...
    PC  = pc;
    int halted;
    if (fscanf(f, "%x %x %u %u %u %d %d", &i, &r, &iff1, &iff2, &im, &halted, &end_tstates2) != 7) {
...
dump_z80_state(void) {
    ...
    printf("%02x %02x %d %d %d %d %d\n", I, R, IFF1, IFF2, IM, context.halted, context.tstates);
}
