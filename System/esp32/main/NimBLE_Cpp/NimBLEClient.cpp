...
NimBLEClient::NimBLEClient(const NimBLEAddress &peerAddress)
    ...
    ble_npl_callout_init(&m_dcTimer, nimble_port_get_dflt_eventq(), NimBLEClient::dcTimerCb, this);
} // NimBLEClient
...
bool NimBLEClient::connect(const NimBLEAddress &address, bool deleteAttributes) {
     ...
     */
    do {
        rc = ble_gap_connect(NimBLEDevice::m_own_addr_type, &peerAddr_t, m_connectTimeout, &m_pConnParams, NimBLEClient::handleGapEvent, this);
        switch (rc) {
            case 0:
                break;
            ...
            default:
                NIMBLE_LOGE(LOG_TAG, "Failed to connect to %s, rc=%d; %s", std::string(m_peerAddress).c_str(), rc, NimBLEUtils::returnCodeToString(rc));
                break;
        }
 ...
 * @param [in] timeout The timeout time in 10ms units before disconnecting.
 */
void NimBLEClient::updateConnParams(uint16_t minInterval, uint16_t maxInterval, uint16_t latency, uint16_t timeout) {
...
bool NimBLEClient::retrieveServices(const NimBLEUUID *uuid_filter) {
    ...
    if (uuid_filter == nullptr) {
        ...
        rc = ble_gattc_disc_svc_by_uuid(m_conn_id, &uuid_filter->getNative()->u, NimBLEClient::serviceDiscoveredCB, &taskData);
    }
 ...
 */
bool NimBLEClient::setValue(const NimBLEUUID &serviceUUID, const NimBLEUUID &characteristicUUID, const NimBLEAttValue &value, bool response) {
...
int NimBLEClient::handleGapEvent(struct ble_gap_event *event, void *arg) {
    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_L2CAP_UPDATE_REQ: {
            ...
            NIMBLE_LOGD(LOG_TAG, "MinInterval: %d, MaxInterval: %d, Latency: %d, Timeout: %d", event->conn_update_req.peer_params->itvl_min, event->conn_update_req.peer_params->itvl_max, event->conn_update_req.peer_params->latency, event->conn_update_req.peer_params->supervision_timeout);
