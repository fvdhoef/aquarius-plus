...
static void insert_char(readline_ctx_t *ctx, char ch) {
...
static void delete_char(readline_ctx_t *ctx) {
    ctx->len--;
...
static void do_left(readline_ctx_t *ctx) {
    if (ctx->idx == 0)
        return;
...
static void do_right(readline_ctx_t *ctx) {
    if (ctx->idx >= ctx->len)
        return;
