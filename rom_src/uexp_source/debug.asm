...
;=============================================================
;
; 2017-05-18 V0.18  lite version - no main menu, no help,
;                   no screen save, uses less screen area,
;                   much smaller RAM footprint.
; 2017-05-20 V0.19  bugfix: showbreakpoints showing wrong addresses.
...
ST_DEBUG:
       ...
       LD    DE,$3000+(40*11)
       CMPHLDE
       jr    nc,.init           ; if cursor inside debug window then
       LD    HL,$3001+(40*11)
...
Break:
       ...
       DEC   HL              ; back up to breakpoint
       LD    (PC_reg),HL     ; save return address (PC at breakpoint)
       EX    (SP),HL         ; restore HL
       INC   SP
...
;       EI
       ...
       OR    A
       SBC   HL,BC           ; break occurred at 5th byte? (single step)
...
.catalog:
       ...
       JR    NZ,.getfilename
       LD    D,10
       LD    E,(IX+win_h)      ; put cursor in middle of window
       SRL   E
       CALL  WinSetCursor
       LD    HL,.nofiles_msg
...
.write:
       ...
       CALL  usb__close_file
       CALL  ShowFileBytes    ; print number of bytes written
       LD    HL,_wr_bytes_msg
       CALL  WinPrtStr
       JP    Wait_Key         ; wait for any key
...
InputFilename:
       ...
       LD    (IY+_flags),1<<CAPSLOCK  ; ? is not help, Q is not quit
       CALL  InputLine        ; input filename
       LD    (IY+_flags),(1<<CAPSLOCK)|(1<<HELPKEY)|(1<<QUITKEY)
...
.showpage:
       ...
       LD    B,UNASMLINES     ; B = lines per page
       JR    .first_line
.next_line:
...
.got_key:
       ...
       CP    "q"              ; Q = quit
       RET   Z
       CP    $03              ; ^C = exit debugger
       JP    Z,_quit
       CP    "c"              ; C = list continuously
       LD    B,1
       JR    Z,.next_line
       JR    .wait_key        ; else eat key
...
TraceHelpMsg:
      db  "RTN step over  . step into  , skip    ",CR
      db  " ; run to RET  : run to BRK ",7," PC-1   ",CR
...
BlankWindow:
      ...
      db   0,1,40,10                     ; x,y,w,h
      dw   0                             ; title

TraceWindow:
      db   (1<<WA_TITLE)|(1<<WA_BORDER)|(1<<WA_CENTER); attributes
      db   GREEN*16+BLACK                ; text colors
      db   WHITE*16+BLUE                 ; border colors
      db   1,2,38,8                      ; x,y,w,h
      dw   trace_title                   ; title
trace_title:
      db   "D E B U G",0

TraceHelpWindow:
      db   (1<<WA_TITLE)|(1<<WA_BORDER)|(1<<WA_CENTER); attributes
      db   WHITE*16+BLUE                 ; text colors
      db   WHITE*16+BLUE                 ; border colors
      db   1,2,38,8                      ; x,y,w,h
      dw   .help_title                   ; title
.help_title:
...
DumpWindow:
      db   (1<<WA_TITLE)|(1<<WA_CENTER)|(1<<WA_BORDER)
      db   WHITE*16+DKBLUE
      db   WHITE*16+BLUE
      db   1,2,38,DUMPLINES
      dw   dm_title
